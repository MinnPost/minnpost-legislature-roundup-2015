...
revisor_url = 'https://www.revisor.mn.gov/laws/?view=session&year=2015&type=0'
bills_list = []
leg_list = {}
...
subject_map = {
  ...
  'Banks and Financial Institutions': 'Business and Economy',
  ...
  'Cities and Towns': 'Administration',
  'Cities and Towns-Specific': 'Administration',
  'Claims': 'Legal',
  'Commerce Department': 'Business and Economy',
  'Commerce and Commerce Department': 'Business and Economy',
  'Commissions': 'Administration',
  'Committees and Working Groups': 'Administration',
  'Constitutional Amendments': 'Administration',
  'Constitutional Offices': 'Administration',
  ...
  'Counties-Specific': 'Administration',
  'Courts': 'Legal',
  'Courts-Specific': '',
  ...
  'Drivers\' Licenses, Training, and ID Cards': 'Administration',
  ...
  'Government-Employees': 'Administration',
  'Government-Federal': 'Administration',
  'Government-Local': 'Administration',
  'Government-State': 'Administration',
  'Governmental Operations-Federal': 'Administration',
  'Governmental Operations-Local': 'Administration',
  'Governmental Operations-State': 'Administration',
  ...
  'Public and State Employees': 'Administration',
  ...
  'Retirement-Public and State Employees': 'Administration',
  'Safety': 'Crime and Drugs',
  'Securities': '',
  'Senior Citizens': 'Social Issues',
  'Sewers and Septic Systems': '',
  'State Agencies and Departments': 'Administration',
  'State Boards': 'Administration',
  'State Councils and Commissions': 'Administration',
  'State Officials': 'Administration',
...
def get_os_bill(data):
  ...
  data['description'] = os_data['title']
...
#Change Revisor date format to OS date format
def make_revisor_date(date):
    ...
    date = date.replace(",", "")
    date_components = date.split(" ")
    month = datemap[date_components[0]]
    day = date_components[1]
    if len(day) < 2:
        day = "0" + day
...
# Get data from revisor's page
def scrape_revisor_page(url):
  html = scraperwiki.scrape(url)
  root = lxml.html.fromstring(html)

  #next_found = get_next(root)

  # Chapter Bill Presentment Date Veto
  for tr in root.cssselect('table.guided tbody tr'):

    # Get bill number and add space
    bill = tr[1].text_content().strip()
    bill = bill[0:2] + " " + bill[2:]
    ...
    veto_link = ''
    for a in tr[3].cssselect('a'):
      veto_link = a.attrib['href']

    data = {
      'chapter': tr[0].text_content()[8:],
      'chamber': chamber,
      'bill': bill,
      'presented': make_revisor_date(tr[2].text_content()),
      'signed': True,
      'vetoed': False,
    ...
    # Handle vetoes
    if tr[3].text_content().strip() not in [None, [], ""]:
        data['signed'] = False
        data['vetoed'] = True
        data['bill_status'] = 'vetoed'

    #Special exception for bill filed without signature
    if bill == "SF 229":
        data['signed'] = False
        data['signed_no_signature'] = True
    ...
    #Use presentment date as final date
    data['end_date'] = data['presented']
...
while (current):
  ...
  current = scrape_revisor_page(current)
